...
try:
    ...
    with tab2:
        ...
        k4.metric("Ticket Promedio ($)", f"${(r_mo/(real_cpus if real_cpus>0 else 1)):,.0f}")
    ...
    with tab3:
        ...
        for c in canales_totales:
            ...
            if v_col:
                ...
                detalles.append({"Canal": c, "Venta Bruta": vb, "Costo": cost, "Margen $": (vb-d-cost), "% Mg": ((vb-d-cost)/(vb-d) if (vb-d)>0 else 0)})
        st.dataframe(pd.DataFrame(detalles).style.format({"Venta Bruta":"${:,.0f}","Margen $":"${:,.0f}","% Mg":"{:.1%}"}), use_container_width=True)
        
        r_cols = st.columns(2)
        with r_cols[0]: st.plotly_chart(px.pie(pd.DataFrame(detalles), values="Venta Bruta", names="Canal", hole=0.4, title="Mix de Venta"), use_container_width=True)
        with r_cols[1]:
            val_stock = float(r_r.get(find_col(data['REPUESTOS'], ["VALOR", "STOCK"]), 0))
            st.metric("VALOR TOTAL STOCK", f"${val_stock:,.0f}")
            st.plotly_chart(px.pie(values=[70, 20, 10], names=["Vivo", "Obsoleto", "Muerto"], hole=0.4, title="Estado del Stock", color_discrete_sequence=["#28a745","#ffc107","#dc3545"]), use_container_width=True)
