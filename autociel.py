...
def cargar_datos_sheets(sheet_id):
    ...
    for h in hojas:
        ...
        # Leemos como texto y limpiamos formatos de moneda/porcentaje de Google
...
try:
    ...
    with tab3:
        ...
        v_neta_t = df_r['Bruta'].sum() - df_r['Desc'].sum()
        mg_p_t = (df_r['Margen $'].sum() / v_neta_t) if v_neta_t > 0 else 0
        c_tot1.metric("Facturación Bruta Total", f"${df_r['Bruta'].sum():,.0f}")
        c_tot2.metric("Margen Total $", f"${df_r['Margen $'].sum():,.0f}")
        c_tot3.metric("Margen Promedio %", f"{mg_p_t:.1%}")
    ...
    with tab4:
        ...
        for i, (nom, row, o_col, sh) in enumerate(sedes_config):
            with (cf1 if i == 0 else cf2):
                f_pur, f_ter, f_rep = row.get('MO Pura', 0), row.get('MO Tercero', 0), (row.get('Fact Repuestos', 0) if nom == 'Salta' else 0)
                st.metric(f"Facturación Total {nom}", f"${(f_pur+f_ter+f_rep):,.0f}")
                st.plotly_chart(px.pie(values=[f_pur, f_ter, f_rep] if f_rep>0 else [f_pur, f_ter], names=["M.O. Pura", "M.O. Terceros", "Repuestos"] if f_rep>0 else ["M.O. Pura", "M.O. Terceros"], hole=0.4, title=f"Composición {nom}"), use_container_width=True)
