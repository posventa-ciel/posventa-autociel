...
def find_col(df, keywords):
    """Busca columna que contenga TODAS las palabras clave"""
...
def cargar_datos(sheet_id):
    ...
    for h in hojas:
        ...
        # 2. Limpieza numérica (SALTANDO FECHAS)
        for col in df.columns:
...
try:
    ...
    with tab2:
        ...
        c_tus = find_col(data['SERVICIOS'], ["OTROS", "CARGOS"])
        
        real_tus = s_r.get(c_cpus, 0) + s_r.get(c_tus, 0)
        ...
        # --- CORRECCIÓN CÁLCULO TICKET PROMEDIO ---
        # Formula: (Hs Facturadas CC + Hs Facturadas CG) / CPUS
        col_h_cc = find_col(data['TALLER'], ["FACT", "CC"])
        col_h_cg = find_col(data['TALLER'], ["FACT", "CG"])
        
        hs_cc = t_r.get(col_h_cc, 0)
        hs_cg = t_r.get(col_h_cg, 0)
        
        divisor = real_cpus if real_cpus > 0 else 1
        tp_hs = (hs_cc + hs_cg) / divisor
        
        # Ticket en $ (Venta MO Cliente + Garantia) / CPUS
        tp_pesos = (s_r.get(c_mc, 0) + s_r.get(c_mg, 0)) / divisor
        
        k3.metric("Ticket Promedio (Hs)", f"{tp_hs:.2f} hs/CPUS")
        k4.metric("Ticket Promedio ($)", f"${tp_pesos:,.0f}/CPUS")

        st.markdown("---")
        # GRÁFICOS RESTAURADOS
        g1, g2 = st.columns(2)
        
        # Gráficos de torta Horas
        col_ht_cc, col_ht_cg, col_ht_ci = find_col(data['TALLER'], ["TRAB", "CC"]), find_col(data['TALLER'], ["TRAB", "CG"]), find_col(data['TALLER'], ["TRAB", "CI"])
        col_hf_cc, col_hf_cg, col_hf_ci = find_col(data['TALLER'], ["FACT", "CC"]), find_col(data['TALLER'], ["FACT", "CG"]), find_col(data['TALLER'], ["FACT", "CI"])
        
        ht_vals = [t_r.get(col_ht_cc, 0), t_r.get(col_ht_cg, 0), t_r.get(col_ht_ci, 0)]
        hf_vals = [t_r.get(col_hf_cc, 0), t_r.get(col_hf_cg, 0), t_r.get(col_hf_ci, 0)]
        ...
        if not hist_s.empty:
            st.plotly_chart(px.line(hist_s, x='Mes', y=c_cpus, title="Evolución CPUS", markers=True), use_container_width=True)
    ...
    with tab3:
        ...
        with rg2:
            c_stock_val = find_col(data['REPUESTOS'], ["VALOR", "STOCK"])
            v_s = float(r_r.get(c_stock_val, 0))
            # Columnas de stock (buscando flexiblemente)
            ...
            # Normalizar porcentaje si viene > 1
            f = 1 if p_vivo <= 1 else 100
            
            df_s = pd.DataFrame({"Estado": ["Vivo", "Obsoleto", "Muerto"], "Valor": [v_s*(p_vivo/f), v_s*(p_obs/f), v_s*(p_muerto/f)]})
        ...
        if not hist_r.empty:
            c_obj_rep = find_col(data['REPUESTOS'], ['OBJ', 'FACT'])
            st.plotly_chart(px.bar(hist_r, x='Mes', y=c_obj_rep, title="Objetivos Mensuales"), use_container_width=True)
    ...
    with tab4:
        ...
        if not hist_cj.empty:
             st.plotly_chart(px.line(hist_cj, x='Mes', y=find_col(data['CyP JUJUY'], ['PAÑOS', 'PROP']), title="Evolución Paños Jujuy", markers=True), use_container_width=True)
