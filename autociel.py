...
def cargar_datos(sheet_id):
    ...
    for h in hojas:
        ...
        # 1. Normalizar nombres de columnas (Quitar puntos y espacios)
        df.columns = [c.strip().upper().replace(".", "") for c in df.columns]
        
        # 2. Limpieza de datos (Evitando columnas de fecha)
...
try:
    ...
    # Procesar Fechas después de la limpieza numérica
    for h in data:
    ...
    def get_mes_data(df):
        ...
        if res.empty:
            return df.iloc[-1]
        return res.sort_values('Fecha_dt').iloc[-1]
    ...
    with tab2:
        ...
        if not hist_s.empty:
            ...
            st.plotly_chart(px.line(hist_s, x='Mes', y=['CPUS', 'TP_PESOS'], title="Evolución Histórica CPUS y Ticket Promedio ($)", markers=True))
    ...
    with tab3:
        ...
        detalles = []
        for c in canales_r:
            v_col, d_col, c_col = find_col(data['REPUESTOS'], ["VENTA", c]), find_col(data['REPUESTOS'], ["DESC", c]), find_col(data['REPUESTOS'], ["COSTO", c])
