...
try:
    ...
    with tab2:
        ...
        ef_cc = hf_vals[0] / ht_vals[0] if ht_vals[0] > 0 else 0
        ef_cg = hf_vals[1] / ht_vals[1] if ht_vals[1] > 0 else 0
    ...
    with tab3:
        ...
        for c in canales_r:
            ...
            if v_col:
                vb = r_r.get(v_col, 0); d = r_r.get(find_col(data['REPUESTOS'], ["DESC", c]), 0)
        ...
        r1.metric("Facturación Bruta", f"${vbt:,.0f}"); r2.metric("Margen Total $", f"${mgt:,.0f}")
        r3.metric("Margen %", f"{mgp:.1%}"); r4.metric("Ticket Prom. Rep.", f"${tp_rep:,.0f}/CPUS")
        ...
        with rg2:
            ...
            p_vivo = float(r_r.get(find_col(data['REPUESTOS'], ["VIVO"]), 0)); p_obs = float(r_r.get(find_col(data['REPUESTOS'], ["OBSOLETO"]), 0))
            p_muerto = float(r_r.get(find_col(data['REPUESTOS'], ["MUERTO"]), 0)); f = 1 if p_vivo <= 1 else 100
    ...
    with tab4:
        ...
        for i, (nom, row, sh) in enumerate([('Jujuy', cj_r, 'CyP JUJUY'), ('Salta', cs_r, 'CyP SALTA')]):
            with (cf1 if i == 0 else cf2):
                ...
                f_p = row.get(find_col(data[sh], ['MO', 'PUR']), 0); f_t = row.get(find_col(data[sh], ['MO', 'TER']), 0)
                ...
                c_ter = row.get(find_col(data[sh], ['COSTO', 'TER']), 0); m_ter = f_t - c_ter
                ...
                if nom == 'Salta':
                    c_rep = row.get(find_col(data[sh], ['COSTO', 'REP']), 0); m_rep = f_r - c_rep
                ...
                # --- PAÑOS POR TÉCNICO (CÁLCULO FORZADO) ---
                col_p_prop = find_col(data[sh], ['PAÑOS', 'PROP'], exclude_keywords=['OBJ'])
                val_p_prop = row.get(col_p_prop, 0)
                ...
                val_tech = row.get(col_tech, 1) # Valor real de la columna técnicos
                # Operación matemática explícita
                if val_tech > 0:
                    ratio = val_p_prop / val_tech
                else:
                    ratio = 0
                
                st.metric("Paños por Técnico", f"{ratio:.1f}")
                st.caption(f"(Basado en {val_p_prop:,.0f} paños y {val_tech:,.0f} técnicos)")
