    ...
    with tab3:
        ...
        for c in canales_r:
            ...
            if v_col:
                ...
                detalles.append({"Canal": c, "Bruta": vb, "Costo": cost, "Margen": (vb-d-cost)})
        st.table(pd.DataFrame(detalles))

    with tab4:
        st.header("Chapa y Pintura")
        f_p_s, f_t_s = cs_r.get(find_col(data['CyP SALTA'], ["MO", "PUR"]),0), cs_r.get(find_col(data['CyP SALTA'], ["MO", "TER"]),0)
        f_r_s = cs_r.get(find_col(data['CyP SALTA'], ["FACT", "REP"]),0)
        st.metric("Facturación Total Salta", f"${(f_p_s+f_t_s+f_r_s):,.0f}")
        st.plotly_chart(px.pie(values=[f_p_s, f_t_s, f_r_s], names=["Pura", "Terceros", "Repuestos"], hole=0.4, color_discrete_sequence=["#00235d", "#00A8E8", "#28a745"]), use_container_width=True)
