...
try:
    ...
    for h in data:
        ...
        col_f = 'Fecha Corte' if 'Fecha Corte' in columnas else ('Fecha' if 'Fecha' in columnas else columnas[0])
        data[h]['Fecha_dt'] = pd.to_datetime(data[h][col_f], dayfirst=True, errors='coerce').dt.date

    # Normalización de nombres de columnas en CALENDARIO para evitar el error técnico
    df_cal = data['CALENDARIO']
    col_dt = next((c for c in df_cal.columns if 'Días' in c and 'Trans' in c), 'Días Transcurridos')
    col_dh = next((c for c in df_cal.columns if 'Días' in c and 'Hábiles' in c), 'Días Hábiles Mes')
    ...
    d_t, d_h = float(c_r[col_dt]), float(c_r[col_dh])
    ...
    with tab1:
        ...
        for i, (tit, real, obj) in enumerate(metas):
            ...
            cols[i].markdown(f"<p style='color:{color}; font-size:18px; margin:0;'><b>Proy: {alc:.1%}</b></p><p style='color:#666; font-size:14px; margin:0;'>Est. al cierre: <b>${p:,.0f}</b></p>", unsafe_allow_html=True)
            cols[i].markdown(f'<div style="width:100%; background:#e0e0e0; height:8px; border-radius:10px; margin-top:5px;"><div style="width:{min(alc*100, 100)}%; background:{color}; height:8px; border-radius:10px;"></div></div>', unsafe_allow_html=True)
    ...
    with tab2:
        ...
        for i, (tit, real, obj) in enumerate([("Total Unidades Servicio (TUS)", real_tus, get_o('SERVICIOS','Obj TUS')), ("Cargos Clientes (CPUS)", s_r['CPUS'], get_o('SERVICIOS', 'Obj CPUS'))]):
            ...
            with (c_u1 if i==0 else c_u2):
                st.markdown(f'<div style="border: 1px solid #e0e0e0; padding: 20px; border-radius: 10px; background-color: white; min-height: 180px;"><p style="font-size: 14px; color: #666; margin-bottom: 2px; font-weight: bold;">{tit}</p><h1 style="margin: 0; font-size: 42px; color: #333;">{real:,.0f} <span style="font-size: 18px; color: #999;">/ Obj: {obj:,.0f}</span></h1><p style="color: {color}; font-weight: bold; font-size: 18px; margin-top: 15px; border-top: 1px solid #eee; padding-top: 10px;">↑ Proyección: {p:,.0f} unidades ({alc:.1%})</p></div>', unsafe_allow_html=True)
        st.markdown("---")
    ...
    with tab3:
        ...
        v_b_t = df_r['Bruta'].sum(); mg_t = df_r['Margen $'].sum(); mg_p_t = mg_t / (v_b_t - df_r['Desc'].sum()) if (v_b_t - df_r['Desc'].sum()) > 0 else 0
        c1.metric("Facturación Bruta Total", f"${v_b_t:,.0f}"); c2.metric("Margen Total $", f"${mg_t:,.0f}"); c3.metric("Margen Promedio %", f"{mg_p_t:.1%}")
        st.markdown("---")
    ...
    with tab4:
        ...
        for i, (nom, row, sh) in enumerate([('Jujuy', cj_r, 'CyP JUJUY'), ('Salta', cs_r, 'CyP SALTA')]):
            r_p, o_p = row['Paños Propios'], get_o(sh, 'Obj Paños Propios Mensual')
            p_p = calcular_proyeccion(r_p, d_t, d_h); alc_p = p_p/o_p if o_p>0 else 0
